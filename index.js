...
bot.action(/^tr_off_(.+)$/, async (ctx) => { await TruckLeasor.findByIdAndUpdate(ctx.match, { status: 'off' }); ctx.reply('የመኪናው ሁኔታ ወደ [❌ ስራ ላይ / የለም] ተቀይሯል።'); ctx.answerCbQuery(); });
...
// የቆዩ የተንጠለጠሉ ግንኙነቶችን (Webhooks) ሙሉ በሙሉ አጽድቶ ማስነሻ
